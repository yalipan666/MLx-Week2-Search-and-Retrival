...
dl = torch.utils.data.DataLoader(train_ds, batch_size=256, shuffle=True, collate_fn=collate_rnn)
dl_val = torch.utils.data.DataLoader(val_ds, batch_size=256, shuffle=False, collate_fn=collate_rnn)

two = models_rnn.Towers(emb=300).to(dev)
torch.save(two.state_dict(), f'./checkpoints/RNN_{ts}.0.0.two.pth')
...
for epoch in range(5):
    ...
    for idx, (qry, pos, neg) in enumerate(prgs):
        ...
        if idx % 50 == 0:
            torch.save(two.state_dict(), f'./checkpoints/RNN_{ts}.{epoch}.{idx}.two.pth')
